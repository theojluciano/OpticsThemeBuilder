...
**The `matchPrimary` lock (`optics-ui`).** The alias above is also a user-facing switch — "Match primary" in Neutral's header, on by default. It covers hue *and* seed lightness, the two seeds Optics actually aliases; saturation is neutral's own. Three things about the implementation:

- **Which families get it.** `canMatchPrimary` in `color-types.ts` answers `id === 'neutral'`, because neutral is the only family Optics aliases. Locking any other family would export a literal anyway, so the switch is not offered there. That function is the single place to widen this.
- **Enforced on write, not derived on read.** `syncMatchedSeeds` runs inside `updateAndSave` / `setAndSave` and on load, copying primary's `h`/`l` into every locked family. So the CSS exporter, the Figma exporter, the previews and the contrast stats all see state whose locked seeds are simply already correct — none of them knows the lock exists. It is idempotent, and a no-op returns the same object identity (which `loadInitialState` relies on to decide whether to write the repair back).
- **The picker releases the lock.** A color chosen from the swatch carries its own hue, so `updateSeedFromPicker` commits the seed *and* `matchPrimary: false` in one patch — two separate calls would let `syncMatchedSeeds` snap the picked hue back before the unlock landed.

**Persisted-state migration.** `SCHEMA_VERSION` / `migrateState` in `color-types.ts`, applied one version at a time (`migrateV1toV2`, `migrateV2toV3`) — each step guards on the version it upgrades *from*, so bumping the schema never re-runs an older repair on state that has already been through it. Tested in `lib/stores/__tests__/migration.test.ts`.

- **v1 → v2.** v1 seeded every family with primary's ramp and had no seed `l`, so saved state carries the wrong curves and the exporter faithfully reports all 57 as deliberate overrides. The repair only replaces a family's ramp when it matches primary's ramp *exactly* (nobody hand-tunes 57 values into another family's curve); a family with one edited stop is left alone, and `h`/`s` are never touched. `importPalette` had the same primary-ramp fallback for the non-imported mode and now falls back per family.
- **v2 → v3.** Adds `matchPrimary`, on for neutral. Saved state predates the concept, so there is no intent to preserve and it simply starts locked — like a fresh theme, and like Optics itself; `syncMatchedSeeds` pulls neutral's `h`/`l` onto primary's on that first load. **Import is the exception**: a tokens file states a hue, so `importPalette` uses `inferMatchPrimary` instead and imports as locked only when the file's neutral seeds already agree with primary's. Covered in `lib/stores/__tests__/match-primary.test.ts`, along with the mirroring behavior and what a locked neutral exports.
